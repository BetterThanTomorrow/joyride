...
(defn default-content-uri [sub-path]
  (vscode-utils/path->uri (vscode-utils/extension-path) (concat ["assets" "getting-started-content"] sub-path)))
...
(defn maybe-create-content+
  ...
  (p/let [exists?+ (vscode-utils/path-or-uri-exists?+ destination-uri)]
    (when-not exists?+
...
(defn maybe-create-user-workspace-activate+ []
  (maybe-create-content+ (default-content-uri ["user" ".joyride" "scripts" "workspace_activate.cljs"])
                         (vscode-utils/path->uri (conf/user-abs-joyride-path) [".joyride" "scripts" "workspace_activate.cljs"])))

(defn maybe-create-user-copilot-instructions+ []
  (maybe-create-content+ (default-content-uri ["user" ".github" "copilot-instructions.md"])
                         (vscode-utils/path->uri (conf/user-abs-joyride-path) [".github" "copilot-instructions.md"])))

(defn maybe-create-workspace-copilot-instructions+ []
