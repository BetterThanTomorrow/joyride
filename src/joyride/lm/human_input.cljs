...
(defn request-human-intelligence!
...
(defn invoke-tool!
  ...
  (fn [^js options ^js _token]
    (let [input (.-input options)
          prompt (.-prompt input)]
      (try
        (p/let [answer (request-human-intelligence! prompt)]
...
(defn register-tool!
  ...
  (try
    (let [tool-impl #js {:invoke (invoke-tool!)}
          disposable (vscode/lm.registerTool "request_human_input" tool-impl)]
      (js/console.log "request_human_input LM Tool registered successfully")
      disposable)
    (catch js/Error e
      (js/console.error "Failed to register request_human_input LM Tool:" (.-message e))
      nil)))
