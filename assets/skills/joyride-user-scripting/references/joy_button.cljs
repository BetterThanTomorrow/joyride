...
(defn ^:export ^:async show-joy-picker! []
  (let [options  [{:iconPath (vscode/ThemeIcon. "go-to-file")
        ...
        selection (await (vscode/window.showQuickPick (clj->js options)
                                                     #js {:title "Welcome, Hacker"
                                                          :placeHolder "What would you like to do?"}))]

    (when selection
      (case (keyword (.-action selection))
        :open-script (do (await (vscode/commands.executeCommand "joyride.openUserScript" "../src/joy_button.cljs"))
                         (await (vscode/commands.executeCommand "joyride.openUserScript" "user_activate.cljs")))
        :copy-examples-link (do (await (vscode/env.clipboard.writeText "https://github.com/BetterThanTomorrow/joyride/tree/master/examples"))
                                (vscode/window.showInformationMessage "Examples link copied to your clipboard" "OK"))
        :copy-video-url (do (await (vscode/env.clipboard.writeText "https://www.youtube.com/watch?v=-yYJV7WEFjI"))
