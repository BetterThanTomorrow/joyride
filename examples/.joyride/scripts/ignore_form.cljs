...
(defn ^:async main []
  (let [editor ^js vscode/window.activeTextEditor
        ...
        _ (await (vscode/commands.executeCommand "paredit.backwardUpSexp"))
    ...
    (if (= "#_" text-before-insert-position)
      (await (eu/delete-range! editor range-before-insert-position))
      (await (eu/insert-text!+ "#_" editor insert-position)))))
